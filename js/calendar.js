...
import { showEventModal, showDayEventsModal, createEventPrompt, updateEventDots } from './event-handlers.js';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    dateClick: function(info) {
      if (isMobile) {
        showDayEventsModal(info.date, info.dateStr, calendar, isMobile);
      } else {
        createEventPrompt(info.dateStr, token);
      }
    },

    eventClick: function(info) {
      console.log('Event clicked:', info.event.title);
      showEventModal(info.event, token, calendar, isMobile);
